...
flowchart TD
    ...
    PanelA <-->|用户同步 / 流量上报| Node1
    PanelB <-->|用户同步 / 流量上报| Node1
    PanelC <-->|用户同步 / 流量上报| Node2
...
- **原因**：TOML 格式规范要求字符串必须包含双引号。
- **解决**：确保例如 `rust_log = "info"` 等属性包含双引号。
...
### 4. 客户端持续报 `Timeout` (超时)
- **原因 1 (混淆不匹配)**：服务端启用了 `obfs`，但客户端未配置相应密码。
- **原因 2 (SNI 异常)**：客户端请求的域名或 SNI 未与服务端证书严格匹配。
- **原因 3 (网络策略)**：服务器防火墙或安全组未放行目标 UDP 端口。
